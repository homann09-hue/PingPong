...
export function buildApp(
  ...
  app.put("/admin/v1/slots/:slotId/availability", async (request, reply) => {
    ...
    await dependencies.liveOpsStore?.recordAudit?.({
      actor: principal.subject, action: "slot.availability.changed", entityType: "slot", entityId: slotId,
      payload: { previousStatus: previous.status, status: current.status, message: current.message },
    }, now);
    return current;
  });
