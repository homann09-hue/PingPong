...
export function PremiumLiveRail() {
  ...
  return <section className="premium-live-layer" aria-label="Live-Casino-Highlights">
    ...
    <section className="premium-jackpot-zone" id="jackpots" aria-labelledby="premium-jackpot-title"><div className="premium-world-header"><div><span>Progressive Pools</span><h2 id="premium-jackpot-title">Jackpots steigen live</h2></div><Link href="/#all-games">Jackpot-Slots</Link></div><div className="premium-jackpot-board">{jackpotPools.map((pool, index) => { const base = Number(pool.value.replaceAll(".", "")); const shown = (base + jackpotTick * (index + 1)).toLocaleString("de-DE"); return <Link href={pool.href} className={`premium-jackpot-pool ${pool.tone}`} key={pool.name}><span className="premium-pool-rays" aria-hidden="true" /><small>{pool.name}</small><strong>{shown}</strong><p><i /> wächst gerade · {pool.game}</p><b><Play weight="fill" /> Jackpot jagen</b></Link>; })}</div></section>
