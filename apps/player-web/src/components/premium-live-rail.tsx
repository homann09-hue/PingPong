...
import { Fire, Gift, Lightning, Play, Sparkle, Trophy, SquaresFour, Crown, Star, ClockCounterClockwise, UsersThree, Heart, CheckCircle, Target, Medal, Timer, ArrowRight, EnvelopeSimple, LockKey, Coins, ChatCircleDots, UserPlus, Broadcast, Confetti } from "@phosphor-icons/react";
...
const friends = [
  { name: "Luna77", game: "Candy Carnival", level: 64, status: "spielt gerade", image: "/assets/ui/player-avatar.png" },
  { name: "MikaX", game: "Neon Nights", level: 51, status: "Jackpot-Runde", image: "/assets/ui/player-avatar.png" },
  { name: "LuckyLeo", game: "Pharaoh Oasis", level: 47, status: "online", image: "/assets/ui/player-avatar.png" },
];
...
export function PremiumLiveRail() {
  ...
  useEffect(() => {
    const timer = window.setInterval(() => setActivePromo((current) => (current + 1) % promos.length), 5200);
    return () => window.clearInterval(timer);
  }, []);
  ...
  return (
    <section className="premium-live-layer" aria-label="Live-Casino-Highlights">
      ...
      <section className="premium-social-promo" id="social" aria-label="Promotions und Freunde">
        <article className={`premium-promo-rotator ${promo.tone}`} key={activePromo}><div className="premium-promo-energy" aria-hidden="true"><i /><i /><i /></div><div className="premium-promo-copy"><span><Broadcast weight="fill" /> {promo.kicker}</span><h2>{promo.title}</h2><p>{promo.copy}</p><Link href={promo.href}><Play weight="fill" /> Jetzt nutzen</Link></div><strong>{promo.reward}</strong><div className="premium-promo-dots">{promos.map((item, index) => <button type="button" key={item.title} className={index === activePromo ? "active" : ""} aria-label={`Promo ${index + 1} anzeigen`} onClick={() => setActivePromo(index)} />)}</div></article>
        <article className="premium-friends-panel"><header><div><span>Deine Casino-Crew</span><h2>Freunde sind online</h2></div><Link href="/#clans"><UserPlus weight="bold" /> Einladen</Link></header><div className="premium-friend-list">{friends.map((friend, index) => <div key={friend.name}><span className="premium-friend-avatar"><Image src={friend.image} alt="" width={48} height={48} /><i /></span><div><strong>{friend.name}</strong><small>Level {friend.level} · {friend.status}</small></div><Link href={index === 2 ? "/#social" : `/slots/${index === 0 ? "candy-carnival" : "neon-nights"}`}><ChatCircleDots weight="fill" /><span>{friend.game}</span></Link></div>)}</div><footer><Confetti weight="fill" /><span>Gemeinsamer Wochenbonus: <strong>68%</strong></span><div><i /></div></footer></article>
      </section>
