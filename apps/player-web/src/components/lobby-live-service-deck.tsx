...
type LobbyState = { streak: number; recentGames: readonly string[]; dismissedInbox: readonly string[] };
...
export function LobbyLiveServiceDeck() {
  ...
  const [lobbyState, setLobbyState] = useState<LobbyState>({ streak: 1, recentGames: [], dismissedInbox: [] });
  ...
  const inboxItems = [
    ...(missionClaimable ? [{ id: `mission:${primaryMission?.id ?? "daily"}`, kind: "mission" as const }] : []),
    ...(claimableAchievements.length ? [{ id: "achievements", kind: "achievements" as const }] : []),
    ...(claimableMilestones.length ? [{ id: "milestones", kind: "milestones" as const }] : []),
  ].filter((item) => !lobbyState.dismissedInbox.includes(item.id));
  const inboxCount = inboxItems.length;
  const grand = jackpots.find((jackpot) => jackpot.tier === "GRAND");

  const unlocked = useMemo(() => games.filter((game) => level >= game.unlockLevel), [level]);
  const featured = useMemo(() => {
    const recent = lobbyState.recentGames.map((id) => unlocked.find((game) => game.id === id)).filter((game): game is (typeof games)[number] => Boolean(game));
    const affinity = recent.flatMap((recentGame) => unlocked.filter((game) => game.category === recentGame.category || game.cabinet === recentGame.cabinet));
    return Array.from(new Map([...recent, ...affinity, ...unlocked].map((game) => [game.id, game])).values()).slice(0, 6);
  }, [lobbyState.recentGames, unlocked]);
  const jackpotGames = useMemo(() => unlocked.filter((game) => game.highRoller).slice(0, 5), [unlocked]);
  const freshGames = useMemo(() => unlocked.filter((game) => game.isNew).slice(0, 5), [unlocked]);

  useEffect(() => {
    void fetch("/api/player/lobby-state", { cache: "no-store" })
      .then((response) => response.ok ? response.json() as Promise<LobbyState> : null)
      .then((state) => { if (state) setLobbyState(state); })
      .catch(() => undefined);
  }, []);
  ...
  async function claimMission() {
    ...
    setClaiming(primaryMission.id); setClaimStatus("idle");
  ...
  async function dismissInbox(id: string) {
    const response = await fetch("/api/player/lobby-state", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ dismissInboxId: id }) });
    if (response.ok) setLobbyState(await response.json() as LobbyState);
  }
  ...
  const streak = Math.min(7, Math.max(1, lobbyState.streak));

  return <section className="lsd" aria-label="Live-Service-Zentrale">
    <div className="lsd__pulse" aria-hidden="true" />
    <header className="lsd__header"><div><span className="lsd__eyebrow"><Sparkle weight="fill" /> Deine Casino-Zentrale</span><h2>Heute wartet mehr auf dich</h2><p>Belohnungen, Wettbewerbe und neue Welten – live mit deinem Fortschritt.</p></div><button className="lsd__inbox" type="button" aria-label="Nachrichten öffnen" aria-expanded={inboxOpen} onClick={() => setInboxOpen(true)}><BellRinging weight="fill" />{inboxCount > 0 && <span>{inboxCount}</span>}</button></header>

    {inboxOpen && <div className="lsd__inbox-popover" role="dialog" aria-modal="true" aria-label="Belohnungs-Inbox"><button className="lsd__inbox-close" onClick={() => setInboxOpen(false)} aria-label="Inbox schließen"><X /></button><span className="lsd__eyebrow"><BellRinging weight="fill" /> Deine Inbox</span><h3>{inboxCount > 0 ? `${inboxCount} Hinweise warten` : "Alles erledigt"}</h3><div className="lsd__inbox-list">
      {inboxItems.map((item) => <div className="lsd__inbox-entry" key={item.id}>{item.kind === "mission" ? <Target weight="fill" /> : item.kind === "achievements" ? <Trophy weight="fill" /> : <Sparkle weight="fill" />}<a href={item.kind === "mission" ? "#missions" : item.kind === "achievements" ? "#rewards" : "#events"} onClick={() => setInboxOpen(false)}><strong>{item.kind === "mission" ? "Tagesmission abgeschlossen" : item.kind === "achievements" ? `${claimableAchievements.length} Erfolge abgeschlossen` : `${claimableMilestones.length} Event-Meilensteine`}</strong><small>{item.kind === "mission" ? `+${coinNumber(primaryMission?.rewardCoins ?? 0)} Coins abholen` : "Belohnungen sind bereit"}</small></a><button onClick={() => void dismissInbox(item.id)} aria-label="Hinweis ausblenden"><X /></button></div>)}
      {inboxCount === 0 && <div className="lsd__inbox-empty"><CheckCircle weight="fill" /><span>Du hast alle aktuellen Hinweise bearbeitet.</span></div>}
    </div></div>}

    <div className="lsd__status-grid">
      <article className="lsd__streak lsd__panel"><div className="lsd__panel-head"><span><Fire weight="fill" /> Daily Streak</span><strong>Tag {streak}</strong></div><div className="lsd__days" aria-label="Tägliche Login-Serie">{[1,2,3,4,5,6,7].map((day) => <span key={day} className={day < streak ? "done" : day === streak ? "current" : ""}>{day < streak ? "✓" : day}</span>)}</div><div className="lsd__reward-row"><Gift weight="fill" /><div><small>Serverseitig gespeichert</small><strong>{streak === 7 ? "Wochenbonus erreicht" : `Noch ${7 - streak} Tag${7 - streak === 1 ? "" : "e"} bis zum Wochenbonus`}</strong></div><a href="#shop">Öffnen</a></div></article>
    ...
    <GameRail title="Für dich" subtitle={lobbyState.recentGames.length ? "Aus deinen zuletzt gespielten Welten" : "Freigeschaltet für dein Level"} icon={<Lightning weight="fill" />} games={featured.length ? featured : games.slice(0,6)} />
